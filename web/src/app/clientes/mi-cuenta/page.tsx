      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="max-w-md text-center p-8">
            ...
            <h1 className="text-xl font-bold text-[#222] mb-2">No encontramos tu cuenta</h1>
            <p className="text-gray-500 mb-6">{authError}</p>
            <div className="flex flex-col gap-2">
              <Link href="/clientes/casas" className="px-6 py-3 rounded-xl bg-[#222] text-white font-medium text-sm text-center">
                Ver casas disponibles
              </Link>
              <button onClick={async () => { await signOut() }} className="px-6 py-3 rounded-xl border border-gray-300 text-sm font-medium text-center hover:bg-gray-50">
                Cerrar sesión
              </button>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[#004274] flex items-center justify-center text-white font-bold text-lg">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-gray-500 hover:text-[#222] hover:bg-gray-100 transition-colors text-sm"
            >
        ...
        {kycRequested && !kycVerified && (
          <div className="mb-6 bg-blue-50 border border-blue-200 rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-[#222] text-sm">Verificación de Identidad Requerida</h3>
                <p className="text-xs text-gray-600">Maninos Capital necesita que verifiques tu identidad.</p>
              </div>
            </div>
            <Link
              href="/clientes/mi-cuenta/verificacion"
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-white text-sm font-medium bg-[#004274] hover:bg-[#00233d] transition-colors"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
                ...
                <h2 className="font-semibold text-[#222]">Mis Compras</h2>
              </div>

              {sales.length === 0 ? (
                <div className="p-10 text-center">
                  <Home className="w-10 h-10 text-gray-300 mx-auto mb-3" />
                  <h3 className="font-semibold text-[#222] mb-1">No tienes compras aún</h3>
                  <p className="text-sm text-gray-500 mb-4">Explora nuestro catálogo y encuentra tu casa ideal</p>
                  <Link href="/clientes/casas" className="text-sm font-medium text-[#004274] hover:underline inline-flex items-center gap-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <div className="min-w-0">
                              <h3 className="font-semibold text-[#222] text-sm truncate">{sale.properties?.address}</h3>
                          ...
                          <div className="flex items-center gap-4 text-sm mt-2 flex-wrap">
                            <span className="text-gray-500">Precio: <strong className="text-[#222]">${sale.sale_price?.toLocaleString()}</strong></span>
                            <span className="text-gray-500">Tipo: <strong className={sale.sale_type === 'rto' ? 'text-[#004274]' : 'text-green-600'}>{sale.sale_type === 'rto' ? 'RTO' : 'Contado'}</strong></span>
                            <span className="text-gray-400 text-xs">{new Date(sale.completed_at || sale.created_at).toLocaleDateString()}</span>
                          </div>
                          ...
                          {sale.status === 'cancelled' && (
                            <div className="mt-3 p-3 rounded-lg bg-red-50 border border-red-100">
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  <p className="font-medium text-sm text-red-700">Solicitud denegada</p>
                                  <p className="text-xs text-red-600 mt-0.5">{sale.rto_notes || 'Tu solicitud no fue aprobada.'}</p>
                                  <Link href="/clientes/casas" className="text-xs font-medium text-red-700 underline mt-1 inline-block">
                                    Ver casas disponibles
                                  </Link>
                                </div>
                              </div>
                          ...
                          {sale.sale_type === 'rto' && sale.rto_monthly_payment && sale.status !== 'cancelled' && (
                            <div className="mt-3 p-3 rounded-lg bg-blue-50 flex items-center justify-between">
                              <div className="flex items-center gap-2 text-sm">
                                ...
                                <span className="font-semibold text-[#004274]">${sale.rto_monthly_payment?.toLocaleString()}/mes</span>
                              </div>
                              {sale.rto_term_months && <span className="text-xs text-gray-500">{sale.rto_term_months} meses</span>}
                            </div>
                          )}
                          ...
                          {sale.status !== 'cancelled' && (
                            <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-4 flex-wrap">
                              {sale.sale_type === 'rto' && (
                                <Link href={`/clientes/mi-cuenta/rto/${sale.id}`} className="text-xs font-medium text-[#004274] hover:underline flex items-center gap-1">
                                  <TrendingUp className="w-3.5 h-3.5" /> Ver contrato RTO
                                </Link>
                              )}
                              {sale.status === 'paid' && (
                                <Link href={`/clientes/mi-cuenta/documentos?sale=${sale.id}`} className="text-xs font-medium text-gray-500 hover:underline flex items-center gap-1">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h2 className="font-semibold text-sm text-[#222] mb-4">Mi Información</h2>
            ...
            {/* Help */}
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h2 className="font-semibold text-sm text-[#222] mb-3">¿Necesitas ayuda?</h2>
              <p className="text-xs text-gray-500 mb-4">Estamos aquí para ayudarte.</p>
              <div className="space-y-2">
                <a
                  href="tel:+18327459600"
                  className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg text-sm font-medium text-[#222] border border-gray-200 hover:bg-gray-50 transition-colors"
                >
            ...
            {/* Quick Links */}
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h2 className="font-semibold text-sm text-[#222] mb-3">Enlaces rápidos</h2>
              <div className="space-y-1">
                {[
                  ...
                  <Link
                    key={link.href}
                    href={link.href}
                    className="flex items-center gap-3 p-2.5 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-[#222] transition-colors group"
                  >
