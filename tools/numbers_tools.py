...
def calculate_maninos_deal(
    ...
    if market_value:
        ...
        if asking_price <= max_offer_70:
            ...
            result["reasoning"].append(f"✅ 70% Rule PASS: Asking Price (${asking_price:,.0f}) is within 70% of Market Value (max: ${max_offer_70:,.0f}).")
        else:
            result["checks"]["70_percent_rule"] = "FAIL"
            result["reasoning"].append(f"⚠️ 70% Rule WARNING: Asking Price (${asking_price:,.0f}) exceeds 70% of Market Value (max: ${max_offer_70:,.0f}).")
    ...
    if arv:
        ...
        if total_investment <= limit_80:
            ...
            result["reasoning"].append(f"✅ 80% Rule PASS: Total Investment (${total_investment:,.0f}) is within 80% of ARV (max: ${limit_80:,.0f}).")
        else:
            result["checks"]["80_percent_rule"] = "FAIL"
            result["status"] = "Rejected"
            result["reasoning"].append(f"🔴 80% Rule FAIL: Total Investment (${total_investment:,.0f}) exceeds 80% of ARV (max: ${limit_80:,.0f}).")
        ...
        try:
            ...
            if fields_to_update:
                ...
                if update_result.get("ok"):
                    logger.info(f"✅ [calculate_maninos_deal] Saved property data: {list(fields_to_update.keys())}")
                else:
                    logger.error(f"❌ [calculate_maninos_deal] Failed to save property data: {update_result.get('error')}")
            
            # 2. UPDATE ACQUISITION STAGE
            new_stage = None
            
            # If 70% rule passed and no 80% check yet (Step 1)
            if result["checks"]["70_percent_rule"] == "PASS" and result["checks"]["80_percent_rule"] is None:
                new_stage = "passed_70_rule"
                logger.info(f"[calculate_maninos_deal] 70% rule passed → updating stage to 'passed_70_rule'")
            
            # If 80% rule passed (Step 4)
            elif result["checks"]["80_percent_rule"] == "PASS":
                new_stage = "passed_80_rule"
                logger.info(f"[calculate_maninos_deal] 80% rule passed → updating stage to 'passed_80_rule'")
            ...
            if new_stage:
                ...
                if stage_result.get("ok"):
                    ...
                    logger.info(f"✅ [calculate_maninos_deal] Acquisition stage updated: {new_stage}")
                else:
                    logger.error(f"❌ [calculate_maninos_deal] Failed to update stage: {stage_result.get('error')}")
        
        except Exception as e:
            logger.error(f"❌ [calculate_maninos_deal] Error updating property: {e}")
