...
class BaseAgent:
    ...
    def __init__(self, name: str, model: str = "gpt-5-mini", temperature: float = 0.7):
...
✅ EJECUTA el flujo completo y confirma el envío"""))
            ...
            logger.info(f"[{self.name}] 🔍 Searching for property_id in {len(messages)} messages")
            for i, msg in enumerate(messages):
                if isinstance(msg, ToolMessage):
                        ...
                        try:
                            ...
                            if isinstance(msg.content, str):
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        logger.info(f"[{self.name}] ✅ Parsed Python dict: {tool_result}")
                                    except (ValueError, SyntaxError) as eval_err:
                                        logger.warning(f"[{self.name}] ❌ Could not parse as JSON or Python dict: {eval_err}")
                                        continue
                            ...
                            if tool_result and isinstance(tool_result, dict):
                                ...
                                if prop_id:
                                    ...
                                    logger.info(f"[{self.name}] 📍 Extracted property_id from {msg.name}: {prop_id}")
                                    break
                                else:
