-- Migration 093: Full reset — combines 091 (wipe) + 092 (opening balances)
-- in one runnable SQL block. Use this when you want to start Test 1 from
-- a clean slate without going through 091 + 092 separately.
--
-- ⚠️  IRREVERSIBLE.  Truncates ALL operational data and re-seeds the
--     opening balance ledger entries for the 6 banks.
...
debits AS (
  ...
  SELECT
    ...
    'Asiento de apertura — migration 093'
  FROM resolved
...
-- (only matters for any UI that still reads bank_accounts.current_balance;
--  the API now derives from the ledger, but we keep this consistent so the
--  table column doesn't display $0 when read directly.)
...
--     Cuenta Conroe        15890.41
--     Cuenta Conroe Cash    2810.50
--     Cuenta Dallas        45237.18
--     Cuenta Dallas Cash    3650.00
--     Cuenta Houston       28492.55
--     Cuenta Houston Cash   4275.89
